...
def _reasons_for_row(row) -> List[str]:
    ...
    if row.get("crime_90d", 0) >= 3:
        reasons.append(f"{int(row['crime_90d'])} LAPD incidents last 90 days")
    if row.get("collision_365d", 0) >= 5:
        reasons.append(
            f"Collision hotspot ({int(row['collision_365d'])} collisions/yr)"
        )
    if row.get("streetlight_30d", 0) >= 1:
        reasons.append(
            f"{int(row['streetlight_30d'])} 311 streetlight outage(s) within 150m"
        )
    if row.get("graffiti_30d", 0) >= 1:
        reasons.append("Graffiti removal request nearby")
    if row.get("dumping_30d", 0) >= 3:
        reasons.append("Elevated 311 illegal dumping reports")
    if not reasons:
    ...
    for row in pdf.itertuples(index=False):
        ...
        out[row.h3] = {
            ...
            "features": {
                ...
                "graffiti_30d": float(row.graffiti_30d),
                "dumping_30d": float(row.dumping_30d),
            },
