...
> **LA Hacks 2026** · Human Impact Challenge

A predictive surveillance intelligence platform for Los Angeles.
Fuses LA Open Data through an NVIDIA RAPIDS pipeline, trains a
per-neighborhood risk forecaster on-device, polls 400+ live Caltrans D7
traffic cameras, and runs every frame through a local vision
...
LA Monitor addresses the safety dimension. LA already publishes
the data — LAPD crime reports, traffic collisions, 311 service requests,
and hundreds of live Caltrans camera feeds. What the city lacks is a
...
### LA Caltrans Catalog (`/pages/la-cameras`)
Searchable 3-column catalog of Caltrans D7 cameras with operator notes,
...
| **cuDF** | GPU dataframe fusion of LAPD + Vision Zero + 311 on H3 cells |
| **cuSpatial / H3** | H3 resolution-9 spatial binning, ~30K hex cells across LA County |
...
```
LA Open Data (SODA)              Caltrans D7 Traffic Cams (364)
      │                                     │
      ▼                                     ▼
...
```bash
# la-monitor brain with CPU fallback
cd poi-brain
python -m venv .venv && source .venv/bin/activate
pip install -r requirements-cpu.txt
...
| `POI_BRAIN_URL` | `http://dgx.tailnet.ts.net:8080` | LA Monitor brain server URL (server-side) |
| `NEXT_PUBLIC_POI_BRAIN_URL` | `http://dgx.tailnet.ts.net:8080` | LA Monitor brain URL (client-side hooks) |
...
adapting Qwen2.5-VL or Qwen3-VL to the LA Monitor detection schema. The
...
app/
  ...
  pages/
    ...
    la-cameras/                          # LA Caltrans catalog + analysis
  ...
  api/
    la-cameras/{cameras,analyze}/        # LA Caltrans proxy + VLM
...
## Data sources (all public, all LA)

| Source | What | Endpoint |
|---|---|---|
| LA Open Data (SODA) | LAPD crime data, traffic collisions, 311 reports | `data.lacity.org` |
| Caltrans D7 TMC | 400+ live traffic camera JPEG streams | `cwwp2.dot.ca.gov/data/d7/cctv` |
...
`webcams.la-cameras.org` (public JPEG snapshots). No PII is stored,
