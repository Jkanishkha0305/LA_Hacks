...
const LaMap = dynamic(() => import("@/components/la-map"), {
  ssr: false,
  loading: () => (
...
export default function MapPage() {
  ...
  const [selected, setSelected] = useState<CaltransCamera | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setIsLoading(true);
        const r = await fetch(`/api/caltrans/cameras?limit=${CAMERA_LIMIT}`);
  ...
  const handleSelect = (camera: CaltransCamera) => {
    setSelected(camera);
    setAnalysis(null);
  ...
  const handleAnalyze = async () => {
    ...
    setAnalysisError(null);
    try {
      const r = await fetch("/api/caltrans/analyze", {
        method: "POST",
      ...
      <div className="mb-4 flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-extrabold uppercase tracking-tight text-deck-fg">
            GRID MAP <span className="text-deck-signal">·</span> LA
          </h1>
          <p className="mt-2 max-w-[60ch] text-[13px] font-medium text-deck-dim">
            Tactical overview of every LA Caltrans camera with a known
