"""Polls LA Caltrans traffic camera snapshots and feeds them into the VLM.
...
def _is_caltrans_placeholder(data: bytes) -> bool:
    ...
    The placeholder is ~16,500-16,550 bytes with a large zero-byte region
    starting around offset 1000.  Real camera frames never have this pattern.
    """
...
async def _fetch_jpeg(client: httpx.AsyncClient, camera: Camera) -> str | None:
    ...
    try:
        ...
        if _is_caltrans_placeholder(resp.content):
            log.debug("[poller] %s returned placeholder image, treating as offline", camera.id)
            return None
