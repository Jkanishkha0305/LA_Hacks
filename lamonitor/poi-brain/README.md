...
LA Monitor — predictive intelligence brain.

FastAPI service that runs on the **Acer Veriton GN100 DGX Spark**. Fuses LA
Open Data through a RAPIDS pipeline, trains a cuML XGBoost risk forecaster,
hosts an NVIDIA NIM Vision model, polls Caltrans D7 traffic cameras, and serves
risk/heatmap/routes endpoints over HTTP + SSE to the Next.js dashboard.

## What it does

```
   LA Open Data (SODA)           Caltrans D7 Traffic Cams
          │                               │
          ▼                               ▼
...
| **cuDF** | Fusing LAPD + collisions + 311 into a single H3-indexed frame |
...
# Run it, binding to port 8000 on all interfaces so la-monitor brain can reach it
docker run -d --name lam-nim \
    --gpus all \
    --shm-size=16g \
    -e NGC_API_KEY=$NGC_API_KEY \
    -p 8000:8000 \
    -v /data/lam/nim-cache:/opt/nim/.cache \
    $NIM_IMAGE
...
### 3. Install LA Monitor brain Python deps
...
docker run --gpus all --network host -it \
    -v $PWD:/workspace/la-monitor-brain \
    -w /workspace/la-monitor-brain \
...
#   SODA_APP_TOKEN=<get a free one at data.lacity.org>
```
...
This pulls LAPD complaints, traffic collisions, 311 street-light reports,
and the Caltrans D7 camera catalog. Writes parquet to `/data/poi/parquet/` and
trains a cuML/XGBoost risk classifier saved to `/data/poi/models/risk_model.pkl`.

### 6. Run LA Monitor brain
...
### 7. Point the Next.js dashboard at the DGX
...
the mission-control dashboard with the H3 heatmap glowing over LA, Caltrans D7
...
# LA Open Data (get a free token at data.lacity.org)
SODA_APP_TOKEN=
...
| GET | `/cameras` | Caltrans D7 camera catalog, tagged with H3 cells |
...
- **Risk granularity**: H3 resolution 9 (~150m hex cells), 15-minute windows.
  LA covered by ~30K cells. Training rows scale with `hex × time` — this is
...
- **Privacy**: nothing leaves the box. NIM is local, RAPIDS is local, camera
  ...
  and webcams.la-cameras.org (public snapshots).
