...
import type { CaltransCamera } from "@/lib/cameras";
...
export default function CamerasPage() {
  const [cameras, setCameras] = useState<CaltransCamera[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState("");
  const [selectedCamera, setSelectedCamera] = useState<CaltransCamera | null>(null);
  ...
  useEffect(() => {
    const loadCameras = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/caltrans/cameras?limit=${CAMERA_LIMIT}`);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch("/api/caltrans/analyze", {
        method: "POST",
      ...
      <div className="mb-6 flex items-end justify-between">
        <div>
          <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.14em] text-deck-dim">
            ...
            <span className="text-deck-signal">/pages/cameras — deck/01</span>
          </div>
          <h1 className="mt-3 text-4xl font-extrabold uppercase tracking-tight text-deck-fg">
            LA CALTRANS <span className="text-deck-signal">·</span> CAMERA INTELLIGENCE
          </h1>
          <p className="mt-2 max-w-[60ch] text-[13px] font-medium text-deck-dim">
            Live feeds from Caltrans District 7 (Los Angeles), piped
