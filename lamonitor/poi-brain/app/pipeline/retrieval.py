...
Stretch: NeMo Retriever embeddings. MVP: text+spatial filter over LAPD
crime data with FAISS vectors of the crime description.
"""
...
class IncidentIndex:
    ...
    Keeps a pandas frame of recent LAPD incidents with lat/lng + description.
    ...
    def load(self) -> None:
        ...
        if not path.exists():
            log.warning("[retrieval] lapd_crime parquet missing, index stays empty")
            return
        ...
        df["dt"] = pd.to_datetime(df.get("date_occ"), errors="coerce")
        df = df.dropna(subset=["dt"])
        df["summary"] = df.get("crm_cd_desc", "incident").astype(str).fillna("incident")
        df["category"] = df.get("crm_cd", "").astype(str).fillna("")
        df["incident_id"] = df.index.astype(str)
        self.df = df[["latitude", "longitude", "dt", "summary", "category", "incident_id"]]
        log.info("[retrieval] loaded %d LAPD rows", len(self.df))
